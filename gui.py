...
def saveSett(login, password, iplist, commands):
    obj = {"login": login, "password": password, "iplist": iplist, "commands": commands}
...
layout = [
    [
        sg.Text("Login:", size=(15, None)),
        sg.Input(
            default_text=login, size=(85, 10), justification="center", key="login"
        ),
    ],
    [
        sg.Text("Password:", size=(15, None)),
        sg.Input(
    ...
    [
        sg.Multiline(
            iplist, key="IP_list", size=(100, 10), justification="left", autoscroll=True
        )
    ],
    [
        sg.Text(
            "Commands list (delimiter enter):", size=(100, None), justification="center"
        )
    ],
    [
        sg.Multiline(
    ...
    # buttons run & stop
    [
        sg.Button("Run", disabled=False, size=(47, None)),
        sg.Button("Stop", disabled=True, size=(47, None)),
    ],
    # output form
    ...
    [
        ...
        sg.Button("Exit", visible=True, size=(47, None)),
    ],
]

# Create the Window
window = sg.Window(
    "Telnet Mass Writer by nikita.baranov@nokia.com", icon="./moduls/img/icon.png"
).Layout(layout)
...
while True:
    ...
    #        break
        ...
        event == sg.WIN_CLOSED or event == "Exit" or None
